...
### 2026 补充：轨迹级 / 工具级评估——不止看结果，还要看过程

本讲的幻觉率和 LLM-as-judge 评的都是**最终输出（outcome）**。但 Agent 是"多步走到答案"的：结果对可能只是运气好，结果错也未必每步都错。2026 的共识是再补一层**过程（process，也叫轨迹级）评估**——检查 Agent 走的每一步是否健康：

| 维度 | outcome 评估（结果级） | process 评估（轨迹/工具级） |
|---|---|---|
| 看什么 | 最终答案对不对 | 中间每步工具调用是否合理 |
| 数据源 | 输入/输出对 | 完整 trace（[Day-48](./Day-48-trace-debugging.md) 采到的就是它） |
| 抓什么问题 | 答错、幻觉 | 绕远路、乱调工具、死循环 |
| 类比 | 只看考试总分 | 按解题步骤给过程分 |

常用的工具级指标（都能从 trace 直接算出来）：

- **工具选择准确率（tool selection accuracy）**：该查库时查库了吗？还是拿计算器去查天气？
- **参数正确性**：工具选对但传参错（错 ID、错格式），照样白跑一趟。
- **调用顺序**：先查后写、先鉴权后操作——顺序错即过程错，哪怕结果碰巧对。
- **无效/冗余调用率**：重复调同一工具、调了却不用结果——不影响对错，但烧钱烧延迟（Day 51 的伏笔）。
- **pass^k 稳定性**：同一任务独立跑 k 次，**全部成功**才算过（区别于 pass@k 的"至少成功一次"）。Agent 输出天然会飘，pass^k 抓的正是"偶尔能对"和"稳定能对"的差距——τ-bench 系列基准用的就是它。

这些指标**不需要新采数据**：Day 48 的 trace 就是轨迹数据源，每个 span 里有工具名、参数、顺序、耗时，写个遍历脚本即可统计。
...
- [ ] 我能区分 outcome 与 process 评估，说出至少三个工具级指标（如工具选择准确率、冗余调用率、pass^k），并知道它们从 Day 48 的 trace 里算。
