...
> **里程碑三档自评**：🟩 跑通（demo 端到端不崩，库内答准、库外拒答）
> → 🟨 能讲清取舍（chunk_size/top-k/阈值为什么取这个值、为什么加引用与拒答、替代方案与代价）
> → 🟥 能扛住追问（失败路径：召回空/引用编造怎么办；成本：每问 token 花多少；安全：文档里被埋注入指令怎么防；规模：文档量上十万块后检索怎么撑）。
>
> **量化亮点卡**：写一张「指标 前→后 + 一句话讲法」
> （例：检索命中率 62%→90%、库外问题拒答 5/5、答案带出处率 100%、调阈值后幻觉回答 3→0），面试直接用。
