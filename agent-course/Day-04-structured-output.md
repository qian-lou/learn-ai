...
> **今日目标**：用 Pydantic + `client.chat.completions.parse` 稳定拿到强类型对象，而不是手解析字符串。
> **时长**：~2h ｜ **前置**：Day 1~3
...
**2026 现代做法**：OpenAI SDK 的 `client.chat.completions.parse(...)`，传入一个 Pydantic 模型作为 `response_format`，SDK 会让模型按该 schema 输出，并**直接反序列化成你的模型实例**（`.message.parsed` 就是对象）。这叫 **Structured Outputs**，底层靠约束解码保证字段齐全、类型正确。（注：新版 SDK 已把 `parse` 从 beta 提升到正式命名空间 `client.chat.completions.parse`，旧的 `client.beta.chat.completions.parse` 仅作兼容别名保留。）
...
- [ ] 我会用 `client.chat.completions.parse(response_format=Model)` 拿对象。
...
- Anthropic 侧：用 tool 的 `input_schema` 约束输出（或其结构化输出 beta 能力），也可在系统提示中给 JSON schema 约束，配合 SDK 解析，思路一致。（确切参数名以 Anthropic 最新文档为准。）
- 关联章节：
