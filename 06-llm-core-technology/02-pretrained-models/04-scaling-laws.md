...
Kaplan 的建议：
  ...
  ⚠️ 2022 Chinchilla 修正：固定算力下应"模型更小 + 数据更多"（计算最优 ≈ 20 tokens/参数），
     GPT-3 实为"训练不足"；后续 LLaMA 等据此用远超 20× 的数据训练较小模型
```
...
*参考答案*：先把预算换算成计算量 C（FLOPs），再用 Chinchilla 的 `D ≈ 20N` 与训练公式 `C = 6ND` 反解。

```
有效算力 = 312 TFLOPS × 0.4 利用率 ≈ 124.8 TFLOPS
C = 124.8e12 × (1000 × 86400 s) ≈ 1.08e22 FLOPs
代入 C = 6·N·D 且 D = 20N  =>  C = 120·N²
N = sqrt(C / 120) = sqrt(1.08e22 / 120) ≈ 9.5e9  ≈ 9.5B 参数
D = 20N ≈ 1.9e11 ≈ 190B tokens
```
结论：约 **9–10B 参数 + ~190B tokens** 是该预算下的 Chinchilla 最优配置。核心公式：固定 C 时 `N_opt ∝ C^0.5`、`D_opt ∝ C^0.5`，且经验上最优 token 数约为参数量的 20 倍。

**练习 2：** 解释为什么 LLaMA-3 (8B) 用 15T tokens 远超 Chinchilla 最优数据量。

*参考答案*：Chinchilla 最优是针对"**固定训练算力下最小化 loss**"的，对 8B 而言最优约 160B tokens（20×8B）。但工业界优化的是**部署期总成本**，而非训练算力。一旦模型要被海量调用，**推理成本 ≫ 训练成本**：与其训一个推理更贵的大模型，不如把小模型"过度训练"（over-training）到 loss 更低。15T tokens 远超 Chinchilla 点，处于收益递减区，多花的训练算力**一次性**投入，却换来一个又小又强、**推理永久更便宜**的 8B 模型。LLaMA-3-8B 性能逼近上一代更大模型，正是这一权衡的体现。简言之：Chinchilla 算的是"训练最划算"，工业界要的是"推理最划算"。

### 进阶题

**练习 3：** 收集 5 个不同大小模型在同一 benchmark 上的分数，拟合 Scaling Law 曲线，预测更大模型的性能。

*参考答案*：直接复用 5. 例题的幂律拟合思路——对 loss 用 `L = a·N^(−b)` 在对数空间线性拟合。

```python
import numpy as np
N = np.array([0.5, 1.5, 7, 13, 70])          # 参数量(十亿)，同系列模型
loss = np.array([2.6, 2.35, 2.05, 1.95, 1.78])  # 同一 benchmark 的 loss
b, log_a = np.polyfit(np.log(N), np.log(loss), 1)
a = np.exp(log_a)
print(f"L = {a:.2f} · N^({b:.3f})")
print(f"预测 175B: {a * 175**b:.3f}")        # 外推到更大模型
```
要点：(1) **必须在对数-对数空间拟合**才呈直线；(2) 用 loss/困惑度这类**平滑指标**外推更可靠，准确率类指标可能因涌现而非幂律；(3) 务必同一模型系列、同一评测协议，否则混入架构/数据差异会污染拟合；(4) 外推幅度不宜过大，远离样本区间误差迅速放大。

**练习 4：** 分析"涌现能力"的经济含义：为什么公司愿意花数千万美元训练一个可能"涌现"新能力的模型？

*参考答案*：经济逻辑是"**可预测的下行 + 不可预测的上行**"。Scaling Laws 让 loss 随规模的下降高度可预测，使数千万美元投入的"基线收益"风险可控——这是敢砸钱的前提。而**涌现能力**（如 CoT 推理、代码、工具使用）只在跨过某规模阈值后突然出现，带来非线性的能力跃迁与潜在的市场垄断/先发优势，构成巨大的期望上行。对头部公司，错过下一次涌现的**机会成本**远高于训练成本；加之首个解锁新能力者可定义产品形态、吸引用户与数据形成飞轮。因此即便单次训练昂贵且能否涌现不确定，期望收益（确定的性能提升 + 小概率的能力突破）仍远超成本——这也是"规模竞赛"的根本驱动。
