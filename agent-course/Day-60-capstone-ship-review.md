...
- [ ] 我的 eval 已沉淀成**可复用的 eval harness**——数据集与断言和本项目解耦，换一个 agent 也能直接对它跑回归。

> **里程碑三档自评**：🟩 跑通（demo 端到端不崩：`docker compose up` 起全栈、三条冒烟全绿）
> → 🟨 能讲清取舍（为什么 liveness/readiness 分离、为什么 eval 进 CI 当门禁而非人工抽查、监控选型的替代方案与代价）
> → 🟥 能扛住追问（失败路径：下游超时/限流触发时行为如何；成本：每千次请求多少钱、怎么降；安全：注入/PII/工具权限三道闸的实证；规模：扩到多副本后限流与指标怎么聚合）。
>
> **量化亮点卡**：写一张「指标 前→后 + 一句话讲法」
> （例：eval 准确率 71%→89%、p95 8s→2.3s、路由降本 60%、注入用例 20/20 拦截），面试直接用。
