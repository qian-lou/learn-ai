...
> **里程碑三档自评**：🟩 跑通（demo 端到端不崩，含第三条"删库"问句）
> → 🟨 能讲清取舍（为什么"取数/定型"分两段、为什么护栏用关键字黑名单而非只靠提示词、替代方案与代价）
> → 🟥 能扛住追问（失败路径：SQL 错/查无/拒答怎么降级；成本：一次查询几轮调用；安全：护栏能否被绕过；规模：换成千表大库怎么办）。
>
> **量化亮点卡**：写一张「指标 前→后 + 一句话讲法」
> （例：10 个自然语言问题自主答对 9/10、写操作用例 3/3 拦截、JOIN 问题零人工提示完成、平均 ≤5 步出结果），面试直接用。
