...
```bash
# 路径 ①：在线动态量化——vLLM 直接把 BF16 权重转 FP8，无需校准、无需 llm-compressor
# Path 1: on-the-fly dynamic FP8 in vLLM (no calibration, no llm-compressor)
vllm serve Qwen/Qwen2.5-7B-Instruct --quantization fp8

# 路径 ②：离线量化——先用 llm-compressor 产出 FP8 checkpoint，再直接 serve 量化目录
# Path 2: offline — produce an FP8 checkpoint via llm-compressor, then serve it directly
vllm serve ./Qwen2.5-7B-Instruct-FP8   # 已是 FP8 权重，无需再加 --quantization
```
...
# 0.9/0.10（2025）起改用配置类，函数式 int4_weight_only() 已弃用 / use Config classes
from torchao.quantization import quantize_, Int4WeightOnlyConfig

# int4 weight-only：仅压权重，激活仍 bf16，走 tinygemm kernel / weight-only, bf16 acts
quantize_(model, Int4WeightOnlyConfig(group_size=128))   # group_size 越小越准、略增体积
# int8 weight-only 同理：quantize_(model, Int8WeightOnlyConfig())
```
...
NF4 (NormalFloat4):
  假设权重服从正态分布 N(0, σ)
  按分位数把概率质量等分成 16 份（每份落入的权重数量期望相等，非等宽划分）
  用各分位点作为 16 个量化值
  → 信息论最优：相比等宽的均匀 INT4，更好地保留了分布信息
