...
ZeRO (DeepSpeed) 分级（按本文 3.4 显存模型：参数:梯度:优化器 = 1:1:4，
共 84GB；口径=4 卡、静态显存，仅切被切部分、其余仍全量驻留）:
  ZeRO-1: 只切片优化器状态        → 84→42GB，   减少 ~50%
  ZeRO-2: 切片优化器 + 梯度       → 84→31.5GB， 减少 ~62.5%
  ZeRO-3: 切片参数 + 梯度 + 优化器 → 84→21GB，   减少 ~75%（思想同 FSDP，通信/实现细节有别）
```
...
```
预期（4 卡，1B 模型，理论静态约 12GB：参数 2GB + 梯度 2GB + Adam 优化器 8GB（FP16 参数/梯度、FP32 的 m+v））：DDP 每卡都存完整的参数/梯度/优化器，峰值显存基本与单卡满载相同；FSDP 把三者切到 4 份，每卡静态约降到 **~1/4**，峰值显存显著低于 DDP（差距随卡数增大），代价是多了 All-Gather/Reduce-Scatter 通信。卡数越多、模型越大，FSDP 省显存越明显。

**练习 4：** 研究 DeepSpeed ZeRO-3 的配置，用 `deepspeed` 训练一个模型。

*参考答案*：DeepSpeed 通过一个 JSON 配置开启 ZeRO-3，训练脚本用 `deepspeed.initialize` 接管模型/优化器。

保存为 `ds_config.json`（标准 JSON 不支持注释，各字段含义见代码块下方说明）：
...
```
字段说明：`stage: 3` 切片 参数+梯度+优化器；`offload_optimizer` 可选，把优化器状态卸载到 CPU；`offload_param` 可选，把参数也卸载到 CPU，进一步省显存。
```python
import deepspeed
