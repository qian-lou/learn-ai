...
1. **Prompt 缓存——白捡的省钱**。2026 的好消息：OpenAI 对 `gpt-4o` 及更新模型**自动开启** prompt 缓存，重复的长前缀（≥1024 token）命中后缓存输入 token **约打五折（≈50% off，半价计费）**，你几乎不用改代码，只要**把静态内容放最前、可变内容放最后**。Anthropic 则是显式 `cache_control`，其缓存读价更低（`cache_read_input_tokens` ≈ 0.1× 输入价，即打一折）。
...
跑两次，第二次的 `cached_tokens` 会 > 0——**那部分 token 计费打了五折（约省一半）**。配合 Day 47 的 trace，你能在面板上直接看到缓存省了多少。
